# PRD: growingStone (Revised)

## 1. 프로젝트 개요

PC 작업자를 위한 **과학 기반 절차적 생성 방치형 위젯 게임**.  
**Second Monitor Game / Desktop Companion** 장르(Rusty's Retirement 등)가 스팀에서 주목받는 시점에서, 틈새 시장을 노린 기획이다.

### 타겟 유저

- 개발자, 디자이너, 학생 등 서브 모니터 활용 유저

### 핵심 철학

- **Scientific:** IMA 데이터 기반
- **Fairness:** P2W 없음, 인게임 경제 100%
- **Zen:** 느린 성장, 관조 중심

### 플랫폼 전략

- **Steam / Epic Games Store:** Windows 전용 ($2 유료)
- **GitHub:** 소스 코드 공개 (무료, 직접 빌드)

### 수익 모델 (BM)

1. $2 판매
2. 제휴 링크 (상세 화면 하단 Reference 형태, 기본 화면 노출 없음)
3. 장기: 실물 표본/굿즈
...
로그인 → 원소 뽑기 → 합성 → 위젯 방치 성장 → 분해/경매/보관 → 반복
...
- 성장은 **로그인 세션 동안에만 발생**
- 서버는 `login_at`, `logout_at` 기록
- 성장량은 서버가 세션 길이 기반 계산
- 클라이언트는 성장값 직접 변경 불가
- **앱을 끈 동안에는 성장이 일어나지 않는다.** 앱이 실행 중일 때만 서버 시간이 흐르며, 그에 따라 질량/부피가 증가한다.

### 3.2 성장 변수 구조

- `growth_time` : 누적 성장 시간
- `size/mass` : 렌더 및 경제 공용 지표
- `render_scale` = f(growth_time)
- 화면 숫자와 실제 렌더 크기는 **항상 일치**

### 3.3 성장 곡선: 로그 함수

- 성장은 **시간에 따라 무한정** 커진다.
- 단, **커지는 속도는 점점 둔화**되어야 한다. 즉 **로그 곡선(log curve)** 으로 성장한다.
- 기본 구조: `size = base + ln(growth_time + 1) * k` (초반 빠름, 후반 완만, 무한 성장 가능)
...
성장은 **원소 증식이 발생하도록 설계**한다.

- 성장 → mass 증가
- 분해 시 mass에 비례해 원소 환원
- 환원율은 100% (손실 없음)

단, 경제 균형을 위해:

> 분해 비용은 항상 같은 양의 원소를 뽑기로 얻는 기대비용보다 높게 설계한다.

성장→분해 전략은 평균적으로 원소 직접 뽑기보다 비용이 높도록 설계한다. 성장의 이득은 희귀 레시피·희귀 원소를 장기적으로 대량화하는 경우에만 의미가 있다.
...
- 보유 중인 돌을 **분해**하면, **현재 mass × 성분비**로 **원소 전량 환원**된다. 키울수록 분해 시 돌아오는 원소량이 많아진다.
- **분해 비용:** `decompose_cost = A * (mass ^ B)` 형태. B > 1, 권장 범위 1.2~1.35. 성장 원소 증식보다 항상 비싸게 유지. 실제로 광물을 원소로 분해하려면 큰 에너지가 든다는 설정. **분해는 "프리미엄 회수"**로, 자주 쓰는 기능이 아님. 테스트 기반 수치 조정.
...
- **성장 정산 시점:** 세션 종료 시 서버가 `growth_time`/`mass`를 정산해 저장한다. 로그인 시에는 무결성 검증만 수행한다. 클라이언트는 성장값을 직접 변경할 수 없다.
- **세션 종료 정의:** 명시적 로그아웃뿐 아니라 앱 종료·강제 종료·서버 타임아웃(마지막 활동 이후 일정 시간 경과)도 포함한다. 서버는 종료 시점을 확정한 후 해당 세션의 성장량을 정산한다.
- **last_seen 갱신:** 앱 실행 중 최소 주기(예: N분) 또는 유의미한 액션(합성/분해/경매/설정 변경) 시 서버에 기록한다. (N은 밸런스·서버비로 조절)
- **logout_at 용도:** 이전 `logout_at`은 오프라인 시간을 성장에 더하기 위한 값이 아니라, 직전 세션 종료 기록의 무결성 체크(세션 이어붙이기/중복 로그인 방지) 용도다.
...
- **기본 슬롯:** 100. 1칸씩 확장, 최대 설계 목표 5000+
- **확장 비용:** `expand_cost(n) = a * (n ^ b)`, b ≈ 1.2~1.3. 5000 도달 시 약 10,000 스톤 수준. 스톤 소각 장치.
- **목적:** DB 안정성, 헤비 유저 싱크, 경제 조절 장치

---

## 9. 경제 및 경매 시스템 (Economy & Auction)

**"하루 한 번 열리는 블라인드 마켓"** — 서버 부하 최소화 및 공정성 확보.

- **화폐:** 경매 입찰·결제는 **인게임 화폐(스톤)만** 사용한다. 현금·외부 결제 없음.
- **수수료:** 5%
- **낙찰 규칙:** 가격 우선. 동일 최고가가 여러 명일 경우, `seed = listing_id + settle_date` 기반의 **서버 결정적 랜덤(Deterministic Random)** 방식으로 1명을 선정한다. 클라이언트 시간은 사용하지 않는다.
...
2. **블라인드 입찰 (Active Time):**
   ...
   - **Security:** RLS(Row Level Security) 정책으로 타인의 입찰/에스크로 내역 조회 불가.
   - 입찰은 즉시 서버에 반영된다. 입찰 시 스톤은 즉시 에스크로 차감되며, 클라이언트는 입찰 결과를 로컬에 보관하지 않는다. 입찰은 서버 타임스탬프 기준으로 기록된다. 입찰 취소는 불가하다.
   - **원자성:** 입찰 처리(에스크로 차감 + top_bid 갱신)는 서버 트랜잭션으로 원자적으로 수행한다.
   - 각 매물은 **정산일**까지 입찰 가능. 기간이 1~7일로 흩어지므로 "마감 시점에 입찰 몰림"이 한날 한 시점에만 집중되지는 않는다.
3. **일괄 정산 (Daily 00:00 KST):**
   - 서버가 `settle_date = 오늘`인 매물만 조회하여 정산한다. 가격 우선으로 최고가를 선정하며, 동일 최고가 다수일 경우 결정적 랜덤으로 낙찰자를 선택한다.
...
- 입찰 로그를 무제한 누적하지 않는다.
- 각 매물은 다음 요약 필드를 유지한다: `top_bid_amount`, `top_bidder_id`, `bid_count`.
- 유저의 에스크로 금액은 별도 구조(`escrow`)로 관리하며, 동일 매물에 대한 추가 입찰은 누적 갱신한다.
- 정산 완료 후, 낙찰 기록만 보존하고 일반 입찰 로그는 삭제한다.
...
- **블라인드 경매** 매물은 등록 즉시 Active 상태가 되며 상시 입찰 가능하다. 정산은 매일 00:00 KST에 일괄 처리된다. 스톤으로만 입찰·결제.
...
## 13. 데이터 구조 (핵심 필드, 프로젝트 스키마 기준)

- **profiles (Phase 1 필수):** id, nickname, minerals (jsonb, stone 잔액 `minerals->>'stone'`), slot_limit, active_stone_id, last_login_at, last_logout_at, created_at, updated_at
- **stones:** id, owner_id, recipe_id, dna (seed·environment 포함), name, birth_date, growth_time, current_mass, created_at, updated_at. `current_mass`는 `growth_time`의 파생 캐시 값이며, 서버가 정산 시 함께 갱신한다.
- **user_materials:** user_id, element, amount (원소 인벤토리)
- **mineral_recipes:** id, name, formula, elements, dna, crystal_system, base_density, base_color, pm_ids, gacha_weight
- **shared_stones:** owner_id, stone_id, stone_definition, title, summary, description, week_key, is_deleted
- **stone_likes:** shared_stone_id, user_id
- **auction_listings (Phase 2):** stone_id, seller_id, min_price, settle_date, status, top_bid_amount, top_bidder_id, bid_count
- **escrows (Phase 2):** user_id, listing_id, amount, updated_at. 유저당 매물당 1 row(업서트)로 누적 갱신. 이벤트 로그가 아닌 상태 테이블.
...
- IMA 기준 광물 종 목록. 유저가 **한 번이라도 합성·획득한** 광물 수집·열람.
- **업적 구간:** 10 / 50 / 100 / 300 / 500 / 1000. 보상: 코스메틱 + 소량 스톤(초반 구간만).

### 돌 자랑/공유 (상세)

- **이미지 저장 안 함.** 레시피·시드 저장, 재렌더링 방식. 좋아요·주간 순위.
...
- **초기:** 무료 티어로 시작. 통신은 **로그인 시, 경매 접속 시, 합성·분해 시** 등 필요한 순간에만 하여 트래픽을 극소화한다. 성장은 로그인 세션 동안만 누적되며, 서버는 로그인/로그아웃 시점을 기록해 세션 길이를 검증한다. 상시 연결이 아니다.
...
현재 구조는 논리적으로 닫혀 있으며, 밸런스 핵심 변수는 **분해 비용 지수 B와 슬롯 확장 지수 b**이다.
