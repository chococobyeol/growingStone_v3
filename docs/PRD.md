...
- **1회분 기준 총량:** 비율을 적용할 때 쓰는 “총량” 단위를 정해야 한다. **30,000**을 사용한다. 총량 300이면 극단적으로 복잡한 화학식에서 최소 계수 쪽이 0.045처럼 1 미만이 되어 10배를 해도 1이 안 된다. 30,000으로 하면 전 광물에서 최소 계수가 1 이상으로 나오고 필요량 숫자도 다루기 좋다. (여유를 더 두려면 100,000 등 확대 가능.) 나누어떨어지지 않을 때는 **소수점 이하 3자리**까지 표시한다.
- **합성 순서 (유지):**
