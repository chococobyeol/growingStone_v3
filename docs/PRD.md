...
- **확률:** 어떤 광물(원소 세트)이 나올지는, **그 광물이 지구에 분포되어 있는 비율(풍부도)** 을 참고하여 정한다. 흔한 광물일수록 나올 확률이 높고, 희귀할수록 낮게 설정한다.
- **PM(Paragenetic Mode) 기반 가중치:** 광물별 확률은 **Paragenetic Mode**(광물 형성 환경, Hazen & Morrison 2022)로 산정한다. 각 광물은 IMA 데이터의 출현 PM 목록(`pm_ids`)을 가지며, PM별 광물 종 수(풍부도 지표)를 합산해 `gacha_weight`를 부여한다. PM47(저온 풍화)처럼 흔한 환경에 출현하는 광물일수록 가중치가 높고, PM11(화산 분기구) 등 희귀 환경에만 나오는 광물일수록 낮다. 상세는 `docs/PM_DISTRIBUTION.md` 참고.
...
| **pm_ids / gacha_weight** | `mineral_recipes`에 저장. `pm_ids`: 해당 광물이 출현하는 PM 번호 목록. `gacha_weight`: PM별 종 수를 합산한 뽑기 가중치. |
